...
export default async function IndustrySolutionsPage({ params }: Props) {
  ...
  const industries = [
    { key: "metallurgy", icon: <Factory className="h-8 w-8" />, title: isZh ? "冶金制造" : "Metallurgy Mfg", desc: isZh ? "钢铁冶金行业数据中台、MES、能源管理、轧辊管理等全链路解决方案。" : "Data middle platform, MES, energy management, roll management for steel and metallurgy." },
    { key: "automotive", icon: <Car className="h-8 w-8" />, title: isZh ? "汽车工业" : "Automotive Industry", desc: isZh ? "汽车供应链管理、工艺文件管理、生产执行系统、质量追溯等数字化方案。" : "Supply chain, process document management, MES, quality traceability for automotive industry." },
    ...
    { key: "logistics", icon: <Truck className="h-8 w-8" />, title: isZh ? "运输物流" : "Transport Logistics", desc: isZh ? "物流一体化平台、运输管理系统、仓储WMS、司机运营管理等解决方案。" : "Integrated logistics platform, TMS, WMS, driver operations management for logistics." },
    { key: "energy", icon: <Zap className="h-8 w-8" />, title: isZh ? "能源环保" : "Energy & Environment", desc: isZh ? "动力电池能源管理平台、碳排放监测、能源效率优化等绿色制造方案。" : "Battery energy management, carbon emission monitoring, energy efficiency optimization." },
    { key: "manufacturing", icon: <Cog className="h-8 w-8" />, title: isZh ? "智能制造" : "Intelligent Mfg", desc: isZh ? "ERP/MES/CRM/SCM/WMS等制造系统开发，适配各类型制造企业需求。" : "ERP, MES, CRM, SCM, WMS development adaptable to all types of manufacturing enterprises." },
  ];
